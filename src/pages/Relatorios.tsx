...
const ReportsViewComponent = ({ setCurrentView }: { setCurrentView: (view: string) => void }) => {
  ...
  const filteredSales = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter((s: Sale) => s.id.toString().includes(t) || s.user.toLowerCase().includes(t) || s.items.some(i => i.name.toLowerCase().includes(t) || (i.sku || '').toLowerCase().includes(t)));
    }
    if (filterPaymentMethod) {
      result = result.filter((s: Sale) => s.payments.some(p => {
        const method = (p.method === 'F12' || p.method === 'Crediário') ? 'Crediário' : p.method;
                ...
                <div className="pt-2 border-t border-slate-200 space-y-1">
                    ...
                    {reprintSale.payments.map((p, i) => (
                        <div key={i} className="flex justify-between text-[10px]">
                            <span>{p.method} {p.installments ? `(${p.installments}x)` : ''}{(p.method === 'F12' || p.method === 'Crediário') ? ` (${p.f12ClientName})` : ''}</span>
