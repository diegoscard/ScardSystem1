...
const SettingsViewComponent = () => {
  ...
  const [localSettings, setLocalSettings] = useState<AppSettings>({ 
    ...
    crediarioInterestRate: settings?.crediarioInterestRate ?? DEFAULT_SETTINGS.crediarioInterestRate,
    crediarioLateFee: settings?.crediarioLateFee ?? DEFAULT_SETTINGS.crediarioLateFee,
    crediarioLateInterestPerDay: settings?.crediarioLateInterestPerDay ?? DEFAULT_SETTINGS.crediarioLateInterestPerDay
  });
        ...
        <div className="space-y-6">
           <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm space-y-8">
              ...
              <div className="space-y-6 border-t pt-6">
                <h3 className="text-lg font-black text-slate-800 uppercase italic border-b pb-4">Taxas Bancárias (%)</h3>
                <div className="grid grid-cols-3 gap-4">
                  {['debit', 'credit1x', 'creditInstallments'].map(key => (
                    <div key={key}>
                      <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">
                        {key === 'debit' ? 'Débito' : key === 'credit1x' ? 'Crédito 1x' : 'C. Parcelado'}
                      </label>
                      <input type="number" step="0.01" onFocus={(e) => e.target.select()} className="w-full border-2 rounded-xl px-4 py-3 text-indigo-600 font-black text-sm" value={(localSettings.cardFees as any)[key]} onChange={e => setLocalSettings({...localSettings, cardFees: {...localSettings.cardFees, [key]: Number(e.target.value)}})} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-6 border-t pt-6">
                <h3 className="text-lg font-black text-slate-800 uppercase italic border-b pb-4">Configurações de Crediário</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Juros do Crediário (%)</label>
                    <input type="number" step="0.01" onFocus={(e) => e.target.select()} className="w-full border-2 rounded-xl px-4 py-3 text-purple-600 font-black text-sm" value={localSettings.crediarioInterestRate} onChange={e => setLocalSettings({...localSettings, crediarioInterestRate: Number(e.target.value)})} />
                  </div>
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Multa por Atraso (%)</label>
                    <input type="number" step="0.01" onFocus={(e) => e.target.select()} className="w-full border-2 rounded-xl px-4 py-3 text-red-600 font-black text-sm" value={localSettings.crediarioLateFee} onChange={e => setLocalSettings({...localSettings, crediarioLateFee: Number(e.target.value)})} />
                  </div>
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Juros Diário (%)</label>
                    <input type="number" step="0.01" onFocus={(e) => e.target.select()} className="w-full border-2 rounded-xl px-4 py-3 text-red-600 font-black text-sm" value={localSettings.crediarioLateInterestPerDay} onChange={e => setLocalSettings({...localSettings, crediarioLateInterestPerDay: Number(e.target.value)})} />
                  </div>
                </div>
              </div>

              <div className="space-y-6 border-t pt-6">
                <h3 className="text-lg font-black text-slate-800 uppercase italic border-b pb-4">Política de Desconto</h3>
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Limite Máximo de Desconto (%)</label>
                  <div className="flex items-center gap-3">
                    <input type="number" step="1" onFocus={(e) => e.target.select()} className="w-32 border-2 rounded-xl px-4 py-3 text-red-600 font-black text-sm" value={localSettings.maxGlobalDiscount} onChange={e => setLocalSettings({...localSettings, maxGlobalDiscount: Number(e.target.value)})} />
                    <span className="text-xs font-bold text-slate-400">Limite apenas para vendedor, administrador não se aplica.</span>
                  </div>
                </div>
              </div>
            </div>
           <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm space-y-6"><h3 className="text-lg font-black text-slate-800 uppercase italic border-b pb-4">Permissões do Vendedor</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{[{id: 'reports_fluxo', label: 'RELATÓRIOS - ENTRADAS/SANGRIAS'}, {id: 'reports_cash', label: 'RELATÓRIOS - HISTÓRICO CAIXA'}, {id: 'stock', label: 'ESTOQUE'}, {id: 'dashboard', label: 'DASHBOARD'}, {id: 'campaigns', label: 'CAMPANHAS'}, {id: 'delete_sale', label: 'EXCLUIR VENDA'}, {id: 'exchange_sale', label: 'REALIZAR TROCA'}, {id: 'fiado', label: 'CREDIÁRIO'}].map(v => (<label key={v.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 cursor-pointer hover:bg-indigo-50 transition-colors"><input type="checkbox" className="w-4 h-4 rounded text-indigo-600" checked={(localSettings.sellerPermissions || []).includes(v.id)} onChange={() => togglePermission(v.id)} /><span className="text-xs font-black text-slate-700 uppercase">{v.label}</span></label>))}</div><p className="text-[9px] font-bold text-slate-400 uppercase italic">Configure o que é relevante para o vendedor acessar.</p></div>
        </div>
      </div>
    </div>
  );
